...
A collection of 76+ Vim/Neovim practice exercises ("katas") forked from `adomokos/Vim-Katas`. Each exercise is a standalone markdown file in `exercises/` that teaches a specific Vim skill through guided drills.

## Structure

- `exercises/` — Numbered markdown files (`001_` through `076_`), each covering one Vim concept (motions, text objects, macros, registers, visual mode, search, etc.)
